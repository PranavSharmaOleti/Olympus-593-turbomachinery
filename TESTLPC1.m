...
%  Updated: N = 5819 rpm, First-Power Swirl, T0 Bisection Solver
% =====================================================================
clear; clc; close all;
...
M1       = 0.549;            % inlet absolute Mach no. (no inlet swirl -> C1=Ca)
...
%% ---------------------------------------------------------------------
%  2. STEP 1 : INLET STATIC CONDITIONS & KINEMATICS
% ------------------------------------------------------------------------
...
U_tip1 = omega * rt1;                     % verify against original 373.56 m/s
...
% ------------------------------------------------------------------------
% First-Power Law: Cw = a*r. 
% SREE integration demands: Ca(r) = sqrt( Ca_mean^2 + 2*(Cw_mean^2 - Cw(r)^2) )
...
for i = 1:N_stages
    ...
    for k = 1:3
        r = radii(k);
        % Swirl velocity proportional to radius
        Cw1_r = Cw1(i) * (r / r_mean(i));
        Cw2_r = Cw2(i) * (r / r_mean(i));
        
        % Axial velocity variation derived from Simple Radial Eq
        Ca1_r = sqrt(Ca^2 + 2*(Cw1(i)^2 - Cw1_r^2));
        Ca2_r = sqrt(Ca^2 + 2*(Cw2(i)^2 - Cw2_r^2));
...
disp('Air angles processed successfully without root flow reversal.');
